...
const ChooseImage = () => {
    ...
    return (
        <SafeAreaView style={{ backgroundColor: "#484848", flex: 1 }}>
            <View>
                <Text style={{ textAlign: "center", marginTop: 15, color: "white", fontSize: 16, fontWeight: "500" }}>Choose Image</Text>
            </View>
            <ScrollView>
                <View style={{ flexDirection: "row", alignItems: "center", flexWrap: "wrap" }}>
                    {images?.map((item, index) => (
                        <Pressable onPress={() => handleSelectImage(item?.image)} style={{ margin: 10 }}>
                            <Image style={{ width: 110, height: 160, resizeMode: "cover", borderRadius: 15 }} source={{ uri: item?.image }} />
                        </Pressable>
                    ))}
                </View>
            </ScrollView>
